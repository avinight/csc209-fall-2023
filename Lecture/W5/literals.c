...
int main(int argc, char **argv) {
    ...
    int rodata_offset = strtol(argv[1], NULL, 16);  // replace
    int rodata_size = strtol(argv[2], NULL, 10);  // replace
    
    // Open the file named in the third argument

    FILE *fp = fopen(argv[3], "rb"); //check if file is NULL to check if the file exists.
    ...
    fseek(fp, rodata_offset, SEEK_SET);
    fread(strings, sizeof(char), rodata_size, fp);

    // Format it for printing
    for (int i = 0; i < rodata_size - 1; i++) {
        if (strings[i] == '\0') {
            strings[i] = '\n';
        }
    }
