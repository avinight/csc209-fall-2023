...
int read_from(int client_index, struct sockname *users) {
    ...
    char message[BUF_SIZE + strlen(username) + 3]; // +3 for ": " and '\0'
    strncpy(message, username, strlen(username) + 1);
    strncat(message, ": ", sizeof(message) - strlen(message) - 1);
    message[BUF_SIZE + strlen(username) + 3] = '\0';

    strncat(message, buf, sizeof(message) - strlen(message) - 1);
    
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (users[i].username) {
            if (num_read == 0 || write(users[i].sock_fd, message, strlen(message)) != strlen(message)) {
