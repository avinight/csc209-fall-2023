...
Bitmap *read_header() {
    ...
    // Read the BMP_HEADER_SIZE_OFFSET bytes
    unsigned char headerData[BMP_HEADER_SIZE_OFFSET];
    fread(headerData, 1, BMP_HEADER_SIZE_OFFSET, stdin);

    // Read the header size into a temporary variable
    int headerSize;
    fread(&headerSize, sizeof(int), 1, stdin);

    // Set the headerSize in the Bitmap structure
    ...
    // Copy the initial header data into the Bitmap header
    memcpy(bmp->header, headerData, BMP_HEADER_SIZE_OFFSET);
    memcpy(bmp->header + BMP_HEADER_SIZE_OFFSET, &bmp->headerSize, sizeof(int));

    // Read the rest of the header data into the Bitmap header
    fread(bmp->header + BMP_HEADER_SIZE_OFFSET + sizeof(int), 1, headerSize - BMP_HEADER_SIZE_OFFSET - sizeof(int), stdin);

    // Read the width and height into the Bitmap structure
...
void scale(Bitmap *bmp, int scale_factor) {
    // Update the width, height, and file size in the header
